...
| **v2.5.1** | **数据持久化**：`sources.json` 存储于 `TRIM_PKGVAR`，更新后用户配置不丢失<br>**客户端自更新**：设置页「检查更新」自动下载 FPK，引导手动安装，支持重新下载<br>**应用类型筛选**：应用列表新增 Docker / 原生应用分类筛选<br>**自过滤**：FN软仓客户端不再显示于应用列表，避免误操作<br>**首页加载优化**：公告数据 5 分钟缓存，页面切换秒开<br>**前端模板拆分**：代码结构更清晰，后续维护更方便<br>**安装向导优化**：新增隐私协议与安全提醒，用户需同意方可继续安装 |
